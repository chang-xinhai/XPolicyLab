...
**Contributor:** XPolicyLab contributors | **Paper:** R3D: Revisiting 3D Policy Learning | **arXiv:** [2604.15281](https://arxiv.org/abs/2604.15281) | **Original code:** [Wushr-Lance/R3D-Policy](https://github.com/Wushr-Lance/R3D-Policy)
...
Download the official pretrained encoder and record its immutable revision/SHA256:

```bash
# Run from the parent workspace, with XPolicyLab importable.
python -m XPolicyLab.policy.pointcloud_common.assets --output /data/model_assets/r3d
# Add to the training command:
# --pretrained-path /data/model_assets/r3d
```

R3D uses XYZRGB. A specified missing checkpoint fails rather than silently using random weights. Omit `--pretrained-path` only for an intentional random-initialization ablation. Deployment restores the full policy checkpoint, so it does not require the external pretrained asset or a network connection. The upstream visual weights omit the positional-encoding Gaussian buffer; this is logged during loading.
