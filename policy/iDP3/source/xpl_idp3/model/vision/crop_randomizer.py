...
import xpl_idp3.model.common.tensor_util as tu
...
class CropRandomizer(nn.Module):
    ...
    def forward_in(self, inputs):
        ...
        if self.training:
            ...
            # center crop by hand
            out = inputs[...,
                (inputs.shape[-2] - self.crop_height) // 2 : (inputs.shape[-2] + self.crop_height) // 2,
                (inputs.shape[-1] - self.crop_width) // 2 : (inputs.shape[-1] + self.crop_width) // 2
            ].clone()
